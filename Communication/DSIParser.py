...
class DSIParser:  # The script contains one main class which handles DSI-Streamer data packet parsing.
    ...
    def get_occipital_lobe_epoch(self, epoch_len_sec):
        occipital_eeg_idx = []
        electrodes = ['O1', 'O2']
        for el in electrodes:
            occipital_eeg_idx.append(self.montage.index(el))
        signalArray = self.get_epoch(epoch_len_sec)
        return signalArray[occipital_eeg_idx, :]  # [#electrodes, epoch_len_sec*samp_freq]
...
#     DSIparser = DSIParser('127.0.0.1', 8844)
#     # DSIparser = DSIParser('localhost', 8944)
#     DSIparser.start()
#     # time.sleep(2)
#     epoch_len_sec = 2
#     for i in range(20):
#         time.sleep(epoch_len_sec / 2)
#         # res = DSIparser.get_epoch(epoch_len_sec)
#         try:
#             print(DSIparser.get_epoch(epoch_len_sec).shape)
#             print(DSIparser.previous_packet_data_timestamp)
#         except:
#             pass
#         # print(
#         #     DSIparser.get_epoch(epoch_len_sec)
#         # )
#     DSIparser.stop()
#     print()
#
# '''
#     for moban in all_moban:
#         rel = CCA(moban, eeg):
#     target = arg_max()
#     action(target)
#
